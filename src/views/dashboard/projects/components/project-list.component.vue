  ...
  <table v-if="projectList.length"
    ...
    <tr>
      <th v-for="(cell, index) in tableHeader"
          ...
          :class="{'text-center' : ['ACTIONS'].includes(cell), 'hidden lg:block' : ['TOPIC'].includes(cell)}">{{ $t(`table.${cell}`) }}</th>
    </tr>
    </thead>
    ...
    <tbody v-auto-animate>
      <tr v-for="(project, index) in projectList"
        ...
        <td class="p-3 truncate w-48">{{project.name}}</td>
        <td class="p-3 truncate w-48">{{project.client}}</td>
        ...
        <td class="p-3 truncate hidden lg:block w-auto">{{project.topic}}</td>
